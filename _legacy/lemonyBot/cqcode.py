...
def DictToCqCode(dict):
    ...
    转换字典为 cqCode json
    """
    return set_cq_code({"type": "json", "data": {"data": DictTocqJsonStr(dict)}})
...
def face(face_id):
    ...
    return set_cq_code({"type": "face", "data": {"id": face_id}})
...
def at(qq, name=""):
    ...
    return set_cq_code({"type": "at", "data": {"qq": qq, "name": name}})
...
def rps():
    ...
    return set_cq_code({"type": "rps", "data": {}})
...
def dice():
    ...
    return set_cq_code({"type": "dice", "data": {}})
...
def shake():
    ...
    return set_cq_code({"type": "shake", "data": {}})
...
def anonymous():
    ...
    return set_cq_code({"type": "anonymous", "data": {}})
...
def share(url, title, content="", image=""):
    ...
    cq_code = {"type": "share", "data": {"url": url, "title": title}}
...
def contact(type, contact_id):
    ...
    return set_cq_code({"type": "contact", "type": {"url": type, "id": contact_id}})
...
def location(lat, lon, title="", content=""):
    ...
    cq_code = {"type": "location", "data": {"lat": "39.8969426", "lon": "116.3109099"}}
...
def music(type, id):
    ...
    return set_cq_code({"type": "music", "data": {"type": type, "id": id}})
...
def image(file, url="", type="", cache=1, show_id=""):
    ...
    cq_code = {"type": "image", "data": {"file": file, "cache": cache}}
...
def node(message, name, uin):
    ...
    return {"type": "node", "data": {"content": message, "name": name, "uin": uin}}
...
def reply(text="", seq="", msg_id="", qq=""):
    ...
    cq_code = {"type": "reply", "data": {"id": msg_id}}
...
def redbag(title):
    ...
    return set_cq_code({"type": "redbag", "data": {"title": title}})
...
def poke(qq):
    ...
    return set_cq_code({"type": "poke", "data": {"qq": qq}})
