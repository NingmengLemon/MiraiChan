...
def generate():
    ...
    for attr_type in detail_moe_attrs.keys():
        if attr_type in res.keys():
            if res[attr_type] in detail_moe_attrs[attr_type]:
                a = random_with_weight(detail_moe_attrs[attr_type][res[attr_type]])
                res[attr_type] += '-'+a
    res["time"] = time.time()
    return res
